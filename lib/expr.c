...
mrb_grn_scan_info_put_index(mrb_state *mrb, mrb_value self)
{
  ...
  scan_info_put_index(ctx, si, DATA_PTR(index), sid, weight);
  return self;
}
...
static mrb_value
mrb_grn_scan_info_setup(mrb_state *mrb, mrb_value self)
{
  int i;
  scan_stat stat;
...
static mrb_value
mrb_grn_scan_info_flags(mrb_state *mrb, mrb_value self)
{
  scan_info *si;
  si = DATA_PTR(self);
  return mrb_fixnum_value(si->flags);
}

static mrb_value
mrb_grn_scan_info_flags_set(mrb_state *mrb, mrb_value self)
{
  int flags;
  scan_info *si;
  si = DATA_PTR(self);
...
static mrb_value
mrb_grn_scan_info_logical_op(mrb_state *mrb, mrb_value self)
{
  scan_info *si;
  si = DATA_PTR(self);
  return mrb_fixnum_value(si->logical_op);
}

static mrb_value
mrb_grn_scan_info_logical_op_set(mrb_state *mrb, mrb_value self)
{
  scan_info *si;
  grn_operator logical_op;
  si = DATA_PTR(self);
...
# define mrb_voidp_value(p) mrb_cptr_value(mrb, (p))
  ...
  mrb_iv_set(mrb, mrb_obj_value(klass), mrb_intern(mrb, "type"),
             mrb_voidp_value(&mrb_scaninfov_type));
  mrb_define_const(mrb, klass, "GRN_OP_OR", mrb_fixnum_value(GRN_OP_OR));
  mrb_define_const(mrb, klass, "SCAN_PUSH", mrb_fixnum_value(SCAN_PUSH));
  mrb_define_const(mrb, klass, "GRN_INVALID_ARGUMENT",
                   mrb_fixnum_value(GRN_INVALID_ARGUMENT));
  mrb_define_const(mrb, klass, "SCAN_START", mrb_fixnum_value(SCAN_START));
  mrb_define_method(mrb, klass, "setup", mrb_grn_scan_info_setup, ARGS_REQ(2));
  mrb_define_method(mrb, klass, "put_logical_op",
                    mrb_grn_scan_info_put_logical_op, ARGS_REQ(3));
  ...
  MRB_SET_INSTANCE_TT(klass, MRB_TT_DATA);
  mrb_iv_set(mrb, mrb_obj_value(klass), mrb_intern(mrb, "type"),
             mrb_voidp_value(&mrb_scaninfo_type));
  mrb_define_method(mrb, klass, "put_index", mrb_grn_scan_info_put_index,
                    ARGS_REQ(3));
  mrb_define_method(mrb, klass, "flags", mrb_grn_scan_info_flags, ARGS_NONE());
  mrb_define_method(mrb, klass, "flags=", mrb_grn_scan_info_flags_set,
                    ARGS_REQ(1));
  mrb_define_method(mrb, klass, "logical_op", mrb_grn_scan_info_logical_op,
                    ARGS_NONE());
  ...
  grn_mrb_eval(ctx,
               ...
               "    i = 0" "\n"
               "    stat = SCAN_START" "\n"
               "    sis = self # todo: need this?" "\n"
               "    si = nil" "\n"
               "    expr.each do |c|" "\n"
               "      i, stat, si = sis.setup expr, var, c, si, i, stat" "\n"
               "      i" "\n"
               "    end" "\n"
               "    return nil unless i" "\n"
               ...
               "      return i" "\n"
               "    else" "\n"
               "      return put_logical_op i, op, e.codes_curr" "\n"
               "    end" "\n"
               "  end" "\n"
               "end" "\n",
               -1);
